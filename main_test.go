package main

import (
	"testing"

	"gopkg.in/h2non/gock.v1"
)

var key = []byte(`-----BEGIN RSA PRIVATE KEY-----

TXAue3jmAcAqjcVjR0j7iRhmyEAMf5ImHFhYaVhp83bXiu893aV4
-----END RSA PRIVATE KEY-----`)

var token = "ghs_16C7e42F292c6912E7710c838347Ae178B4a"

func Test_fetchTokenSuccess(t *testing.T) {
	defer gock.Off()
	gock.New("https://api.github.com/app/installations/1/access_tokens").
		MatchHeader("Accept", "application/vnd.github.v3+json").
		MatchHeader("Authorization", "^Bearer .*$").
		Reply(201).
		JSON(map[string]string{"token": token})

	value, _ := fetchInstallationToken("1", "2", key)

	if value != token {
		t.Errorf("Expected %s, got %s", token, value)
	}
}

func Test_privateKeyIsInvalid(t *testing.T) {
	_, err := fetchInstallationToken("1", "2", []byte("not a rsa key"))

	if err == nil {
		t.Errorf("Expected failure, program ran through")
	}
}

func Test_fetchFailureStatusCode(t *testing.T) {
	type args struct {
		installationID string
		appID          string
		privateKey     []byte
	}
	tests := []struct {
		code    int
		args    args
		want    string
		wantErr bool
	}{
		{code: 401, args: args{installationID: "1", appID: "2", privateKey: key}, want: "401", wantErr: true},
		{code: 404, args: args{installationID: "1", appID: "2", privateKey: key}, want: "401", wantErr: true},
		{code: 500, args: args{installationID: "1", appID: "2", privateKey: key}, want: "401", wantErr: true},
	}
	for _, tt := range tests {
		t.Run("testing", func(t *testing.T) {
			gock.New("https://api.github.com/app/installations/1/access_tokens").
				MatchHeader("Accept", "application/vnd.github.v3+json").
				MatchHeader("Authorization", "^Bearer .*$").
				Reply(tt.code)

			_, err := fetchInstallationToken(tt.args.installationID, tt.args.appID, tt.args.privateKey)
			if (err != nil) != tt.wantErr {
				t.Errorf("fetchInstallationToken() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
		})
	}
}
